#!/usr/bin/env bash
# Inference only: hand-tuned, Bayesian, PPO5005 best-evaluation, and ARS4218.
...
RESULTS_DIR="${RESULTS_DIR:-$PROJECT_ROOT/real_demo/real_demo/eval_box_lift_all_methods_b50_150_250_$(date +%Y%m%d_%H%M%S)}"
...
for method in "${METHODS[@]}"; do
    if [[ "$method" == "handtuned" || "$method" == "bayesian" ]]; then
